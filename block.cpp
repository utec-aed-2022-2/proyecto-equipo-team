...
//    Block(int index, vector<Transaction> transactions, string* prev_hash_code){ // [this need to change when Transaction is fully implemented, we are assuming string for simplicity]
    ...
    string generate_hash_code(){
        ...
        hash_code = sha256( to_string(index) + to_string(nonce) + transact + (*prev_hash_code) );
        return hash_code; // missing the rest of members of data
    }
...
//    }
    ...
    bool is_valid(){
        string aux = generate_hash_code();
        cout << hash_code << endl;
        cout << aux << endl << endl;
        return (hash_code == aux);
    }
